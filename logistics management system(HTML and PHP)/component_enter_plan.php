...
<html>
   <?php
        ...
        $component_query="select * from component;";
        $component_result=$db->query($component_query);
		$provider_result=$db->query($provider_query);
        echo "<script>var i=0; var component_id=new Array(); var component_name=new Array();
			var enter_person=new Array(); var provider=new Array(); 
			var component_standard=new Array(); var component_unit=new Array();
			 </script>";
        while($component_row=mysqli_fetch_assoc($component_result)){
            echo "<script>component_standard[i]='".$component_row['component_standard']."';
					component_unit[i]='".$component_row['component_unit']."';
					component_id[i]='".$component_row['component_id']."';
					component_name[i]='".$component_row['component_name']."';
					i++;</script>";
		}
		echo "<script>i=0;</script>";
        $component_result=$db->query($component_query);
		$enter_person="select name from account where dpt_id=2;";
        $enter_person_result=$db->query($enter_person);
 		while($enter_person_row=mysqli_fetch_assoc($enter_person_result)){
            echo "<script> 
                enter_person[i]='".$enter_person_row['name']."';
                i++;</script>";
        }
		echo "<script>i=0;</script>";
		$enter_person_result=$db->query($enter_person);
		...
		while($provider_row=mysqli_fetch_assoc($provider_result)){
            echo "<script> 
                provider[i]='".$provider_row['provider_name']."';
                i++;</script>";
        }
		echo "<script>i=0;</script>";
		$provider_result=$db->query($provider_query);
   ...
   <div class="row clearfix">
      <div class="col-md-12 column">
         ...
         <div class="row clearfix">
            ...
            <div class="col-md-10 column">
            <h3>component_enter_plan_table</h3>
                  ...
                  <tr>
                    ...
                    <th>
                     ...
                     <select>
                        <?php
                            while($component_row=mysqli_fetch_assoc($component_result)){
                            	echo "<option value='".$component_row['component_id']."'>".$component_row['component_id']."</option>";
                            }
							$component_result=$db->query($component_query);
                        ?>
                    </select>
                   ...
                   <?php
                            while($enter_person_row=mysqli_fetch_assoc($enter_person_result)){
                            	echo "<option value='".$enter_person_row['name']."'>".$enter_person_row['name']."</option>";
                            }	
							$enter_person_result=$db->query($enter_person);
                        ?>
                    </select>
   ...
   function confirm(){
     ...
     for(var i=1;i<rows.length;i++)
     {
		...
		alert(postPara);
    	AJAX('component_enter_plan_exe.php',postPara,function(str){
			alert(str);		
		});
   ...
   function edit(){
         ...
         for(var i=1;i<rows.length;i++)
         {
            for(var j=0;j<rows[i].cells.length;j++)
            {
                ...
                if(j==0){
                   // alert(provider_name.length());
                     for(var k=0;k<provider.length;k++){
                        ...
                        option.value=provider[k];
                        option.text=provider[k];        
                    }
                }
                if(j==1){
                     for(var k=0;k<100;k++){
                ...
                if(j==3){
                    for(var k=0;k<component_name.length;k++){
                        var option=document.createElement('option');
                        select.options.add(option);
                        option.value=component_name[k];
                        option.text=component_name[k];        
                    }
                }
                if(j==4){
                   	for(var k=0;k<100*100;k+=100){
                        var option=document.createElement('option');
                        select.options.add(option);
                        option.value=k;
                        option.text=k;      
				...
				if(j==5){
					for(var k=0;k<enter_person.length;k++){
                        var option=document.createElement('option');
                        select.options.add(option);
                        option.value=enter_person[k];
                        option.text=enter_person[k];        
                    }
				}
                if(j==6){
                    for(var k=0;k<2;k++){
