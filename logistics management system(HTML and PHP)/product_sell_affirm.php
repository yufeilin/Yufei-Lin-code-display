...
<html>
   <?php
        ...
        $query="select * from purchase_affirm;";
        ...
        while($row=mysqli_fetch_assoc($result)){
			$provider_name[$i]=$row['provider_name'];
			$purchase_id[$i]=$row['purchase_id'];
			$component_id[$i]=$row['component_id'];
			$purchase_count[$i]=$row['purchase_count'];
			$buyer[$i]=$row['purchase_person'];
			$is_arrived[$i]=$row['is_arrived'];
			$i++;
        }
   ...
   <div class="row clearfix">
      <div class="col-md-12 column">
         ...
         <div class="row clearfix">
            ...
            <div class="col-md-10 column">
            <h3>product_sell_affirm_table</h3>
               <table class="table table-bordered table-hover" id="plan_table">
				...
				<?php 
					echo '<tbody>';
					for($j=0;$j<$i;$j++){
						echo "<tr><th>".$provider_name[$j]."</th>";
						echo "<th>".$purchase_id[$j]."</th>";
						echo "<th>".$component_id[$j]."</th>";
						echo "<th>".$purchase_count[$j]."</th>";
						echo "<th>".$buyer[$j]."</th>";
						echo "<th>".$is_arrived[$j]."</th>";
	...
	function check(){
		...
		for(var i=1;i<rows.length;i++)
		{
			...
			if(box.checked==true){
				...
				postPara="purchase_id="+rows[i].cells[1].innerHTML;
				AJAX('purchase_affirm_exe.php',postPara,function(str){
					alert(str);		
				});
			}
		}
