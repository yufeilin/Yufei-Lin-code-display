...
<html>
   <?php
        ...
        $query="select * from product_coding;";
        $result=$db->query($query);
        echo "<script>var i=0; var product_id=new Array(); var product_name=new Array();
			var buyer=new Array();</script>";
        while($row=mysqli_fetch_assoc($result)){
            echo "<script>product_id[i]='".$row['product_id']."';
   ...
   function confirm(){
     ...
     for(var i=1;i<rows.length;i++)
     {
		...
		alert(postPara);
    	AJAX('product_enter_plan_exe.php',postPara,function(str){
			alert(str);		
		});
   ...
   function edit(){
         ...
         for(var i=1;i<rows.length;i++)
         {
            for(var j=0;j<rows[i].cells.length;j++)
            {
                ...
                if(j==1){
                    for(var k=0;k<product_id.length;k++){
                        var option=document.createElement('option');
                        select.options.add(option);
                        option.value=product_id[k];
                        option.text=product_id[k];        
                    }
                }
                if(j==2){
