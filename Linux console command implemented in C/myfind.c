...
int matchMtime(struct stat stat_buf,int n){/* {{{ */
	...
	printf("time now:\t%s\n",ctime(&now));
	printf("difftime:\t%f\n",difftime(now,mtime));
	printf("day:\t%d\n",(int)difftime(now,mtime)/(60*60*24));
	printf("********************\n");*/
...
int matchCtime(struct stat stat_buf,int n){/* {{{ */
	...
	printf("time now:\t%s\n",ctime(&now));
	printf("difftime:\t%f\n",difftime(now,ctime_));
	printf("day:\t%d\n",(int)difftime(now,ctime_)/(60*60*24));
	printf("********************\n");*/
...
char* getPath(char* path,char* filename){/* {{{ *///根据目录名和目录下的文件名获取文件路径
...
void initArgvPosition(int argc,char*argv[]){//确定参数的位置 /* {{{ */
	int i;
	if(argv[2][0]!='-'){//第三个要求以'-'开头
		printHelp();
		exit(EXIT_FAILURE);
	}
	for(i=2;i<argc;i++){
...
void findFunc(int argc,char* argv[],char * path){//find主程序/* {{{ */
	...
	do{//读取目录下的文件，直到读完为止
		...
		if((dir_struct=readdir(dir))!=NULL){//读取目录下当前文件
			...
			if(mtimeToggle==TRUE){//-mtime
				if(!matchMtime(stat_buf,atoi(argv[mtimePosition+1])))
					matchToggle=FALSE;
			}
			if(ctimeToggle==TRUE){//-ctime
